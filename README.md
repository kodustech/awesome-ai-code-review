...
- **[BrotherSBE](https://github.com/khalilmaaouni/BrotherSBE)** - Claude Code plugin that reviews backend and data system designs before code is written, holding the result to gates that read receipt files (numbers, migrations, approvals, test runs) and answer NO-DATA rather than passing on absent evidence. Free, MIT licensed.
